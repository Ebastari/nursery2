...
import BibitDistributionProgress from '../components/BibitDistributionProgress';
...
export function ApprovalScreen() {
  ...
  const [approving, setApproving] = useState<string | null>(null);
    ...
    const handleApprove = async (item: SuratJalanItem) => {
      ...
      try {
        await approveSuratJalan(item.nomor, isAdmin ? 'Admin' : 'User');
      } catch (err: any) {
  ...
  return (
    <div className="fade-in space-y-4 pb-24">
      {/* Progress Distribusi Bibit Dashboard */}
      <div className="max-w-xl mx-auto">
        <BibitDistributionProgress
          data={items.map((item) => ({
      ...
      {Array.isArray(items) && items.length === 0 ? (
        ...
        <div className="space-y-3">
          {Array.isArray(items) && items.map((item, idx) => {
            ...
            const doc = Array.isArray(documents) ? documents.find((d) => d.nomor === item.nomor) : undefined;
            return (
                    ...
                    <>
                      <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-green-50 border border-green-200">
                        ...
                        <span className="text-xs text-green-700">
                          Disetujui oleh {approval.approvedBy} pada {approval.approvedAt ? new Date(approval.approvedAt).toLocaleString('id-ID', { 
